...
Home Assistant add-on that provides a dedicated chat UI and backend proxy for the Codex Mule relay.
...
- Compatible with `codex_mule_conversation` custom Assist agent integration (native Assist routing to Codex Mule).
...
- `notify_webhook_id`: Webhook id used by `/api/ha/notify` (default: `velox_codex_mule_webhook`)
...
Second-user variant:
- Create a second automation only if you want Codex Mule to fan out to another device/user.
