...
LOG = logging.getLogger("codex-mule-addon")
APP_DISPLAY_NAME = "Codex Mule"
...
class Settings(BaseModel):
    ...
    assist_agent_id: str = ""
    assist_language: str = ""
    notify_webhook_id: str = "velox_codex_mule_webhook"
...
app = FastAPI(title=f"{APP_DISPLAY_NAME} Add-on", version=APP_VERSION)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
class HaNotifyBody(BaseModel):
    message: str
    title: str | None = APP_DISPLAY_NAME
    level: str | None = "info"
...
async def api_ha_notify(body: HaNotifyBody) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "title": (body.title or APP_DISPLAY_NAME).strip() or APP_DISPLAY_NAME,
        "message": message,
...
async def startup_log() -> None:
    ...
    settings = load_settings()
    LOG.info(
        "Codex Mule add-on started relay_url=%s relay_token_present=%s wait_timeout=%s poll_interval=%s",
