"""Conversation agent for Codex Mule via Codex relay."""
...
async def async_setup_entry(
    ...
    """Set up Codex Mule conversation entity."""
    async_add_entities([CodexMuleConversationAgent(hass, config_entry)])


class CodexMuleConversationAgent(ConversationEntity, conversation.AbstractConversationAgent):
    """Codex Mule-backed conversation agent."""
    ...
    async def async_process(self, user_input: ConversationInput) -> ConversationResult:
        ...
        try:
            ...
            # control/intents work natively. Fall back to Codex Mule relay only when HA
            # reports no intent match.
            try:
                ha_result = await self._ha_builtin_process(user_input, conv_id)
            except Exception as err:
                _LOGGER.debug("HA built-in routing unavailable, continuing with Codex Mule relay: %s", err)
                ha_result = None
            ...
            _LOGGER.exception("Codex Mule conversation failed")
            response.async_set_error(
                intent.IntentResponseErrorCode.UNKNOWN,
                f"Codex Mule agent error: {err}",
            )
    ...
    async def _ha_builtin_process(self, user_input: ConversationInput, conv_id: str) -> ConversationResult | None:
        ...
        # Fall back to Codex Mule relay only when HA explicitly reports no intent match.
